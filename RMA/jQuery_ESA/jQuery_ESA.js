...
jQuery(document).ready(function ($) {
    ...
    function createNewItem(event) {
        ...
        var $newItem = $('<li> <input type="checkbox"/> <label> <input type="text" readonly/></label> <div class="arrow-btn"></div></li>');
        $newItem.append($('<ul class="item-menu-bar"> <li class="edit-btn">Editieren</li> <li class="duplicate-btn">Duplizieren</li> <li class="remove-btn">Löschen</li> </ul>'));
        $('ul.todo-list').append($newItem);

        //Checkbox stylen und Attribute setzen
        var $cb = $newItem.find('input[type="checkbox"]');
        styleCheckbox($cb);
        setCheckboxAttributes($cb, uniqueItemID++); //checkbox attribute neu setzen
                                                    //für korrekte checkbox-label Zugehörigkeit
        ...
        $newItem.find('.duplicate-btn').on('click', function () {
            var $duplicate = duplicateItem($(this.closest('ul.todo-list > li')));
            /*Item Editier Modus beenden vor Erzeugen*/
            if (editItem) {
                unEditModeItem();
            }

            fadeInItem($duplicate);
        });
