<?php
  // redirect to the projects page if the user is logged in; display the login
  // screen otherwise
  session_start();
  require "php/vendor/autoload.php";
  include 'php/datasource/mongo_db_datasource.php';
  if($_SERVER['REQUEST_METHOD'] === "POST"){

    // We borrow the Datasource implementation for mongodb from the strategy
    // pattern in the request handling for REST resources - this here is _not_
    // a design pattern 
    $datasource = new MongoDbDatasource();
    $users = $datasource->retrieve("User", array("username" => $_POST["username"]));

    if (count($users) == 1 && password_verify($_POST["password"], $users[0]->password_hash)){
      $_SESSION["user_id"] = $users[0]->id;
      $_SESSION["username"] = $users[0]->username;
      header("Location: /isp/esa3/projects.php");
      die();
    }
  }
?>
