...
$("#projects").change(function () {
  ...
  $("#pdescription").val("");
  if(this.value != "0"){
    $.get("/isp/esa3/php/resources/project.php?id=" + this.value, (result) => {
...
$("#psave").click(function () {
  ...
  if ($("#projects").val() === "0"){
    $.ajax({
            url: "/isp/esa3/php/resources/project.php",
            type: "POST",
...
function initProjects(selectedValue) {
  $.get("/isp/esa3/php/resources/project.php", (result) => {
...
$("#requirements").change(function () {
  ...
  initLabels("r_");
  if(this.value != "0"){
    $.get("/isp/esa3/php/resources/requirement.php?id=" + this.value, (result) => {
      result = JSON.parse(result);
...
$("#rsave").click(function () {
  ...
  if ($("#requirements").val() === "0"){
    $.ajax({
            url: "/isp/esa3/php/resources/requirement.php",
            type: "POST",
            data: JSON.stringify(object),
            success: function(result){
              result = JSON.parse(result);
...
function initRequirements(projectId, selectedValue){
  if(projectId === "0") {
    ...
    $.get("/isp/esa3/php/resources/requirement.php?project_id=" + projectId, (result) => {
